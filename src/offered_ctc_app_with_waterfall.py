...
if st.button("Predict Offered CTC"):
    try:
        ...
        st.subheader("🔍 Top 10 SHAP Feature Contributions")
        ...
        waterfall_features = top_features.copy()
        waterfall_values = list(top_shap_vals)
        remaining_contribution = contribution_sum - sum(waterfall_values)
        if abs(remaining_contribution) > 1e-6:
            waterfall_features.append("Other Features")
            waterfall_values.append(remaining_contribution)

        measure = ["relative"] * len(waterfall_values) + ["total"]
        x_labels = waterfall_features + ["Final Prediction"]
        y_values = waterfall_values + [final_prediction - base_value]
        ...
        fig_waterfall.update_layout(
            title="💧 SHAP Waterfall: From Base CTC → Feature Contributions → Final Prediction",
            height=600
        )
