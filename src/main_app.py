...
from utils import (
    ...
    plot_global_shap  # Make sure you have added this in utils.py
)
...
st.title("💼 Offered CTC Prediction & Model Explainability")

# Tabs for Local vs Global Explainability
tab_local, tab_global = st.tabs(["🔍 Local Explainability", "🌍 Global Explainability"])
...
with tab_global:
    ...
    st.info("This section shows how features impact predictions across the entire dataset (global SHAP).")

    # Upload dataset or load from path
    dataset_option = st.radio("Choose dataset source:", ["Upload CSV", "Load from file path"])

    if dataset_option == "Upload CSV":
        uploaded_file = st.file_uploader("Upload training dataset", type=["csv"])
        if uploaded_file is not None:
            full_df = pd.read_csv(uploaded_file)
            plot_global_shap(rf_model, preprocessor, full_df)
    else:
        dataset_path = st.text_input("Enter dataset path:", r"C:\path\to\training_dataset.csv")
